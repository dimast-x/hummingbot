...
from typing import TYPE_CHECKING, Optional, List #, Dict, Any, AsyncIterable, Tuple
...
# class PeatioExchangeTransactionTracker(TransactionTracker):
#     def:
...
#     def c_did_timeout_tx(self, str tx_id):
...
class PeatioExchange(ExchangeBase):
    ...
    def __init__(self, client_config_map: "ClientConfigAdapter", peatio_api_key: str, peatio_api_secret: str, trading_pairs: Optional[List[str]] = None, trading_required: bool = True):
   
        super().__init__(client_config_map)
        self._trading_pairs = trading_pairs
        self._peatio_auth = PeatioAuth(peatio_api_key, peatio_api_secret)
        self._order_books = {}
        self._peatio_api_key = peatio_api_key
        self._peatio_api_secret = peatio_api_secret
        self._trading_required = trading_required
        
        if not self._peatio_api_key:
            raise ValueError("API key required")

        if not self._peatio_api_secret:
    ...
    async def all_trading_pairs(self) -> dict:
        trading_pairs = await self._api_request("GET", "/api/v2/markets.json")
        return {tp["name"]: tp for tp in trading_pairs}
    ...
    async def get_balance(self) -> dict:
        balances = await self._api_request("GET", "/api/v2/members/me.json")
        return {balance["currency"]: balance["balance"] for balance in balances["accounts"]}

    async def get_order_book(self, trading_pair: str) -> dict:
        order_book = await self._api_request("GET", f"/api/v2/order_book.json?market={trading_pair}")
        return order_book

    async def get_last_trade_price(self, trading_pair: str) -> float:
        trades = await self._api_request("GET", f"/api/v2/trades.json?market={trading_pair}")
        if len(trades) > 0:
            return trades[0]["price"]
        else:
            return 0.0

    async def create_order(self, trading_pair: str, order_type: str, price: float, quantity: float, **kwargs) -> dict:
        ...
        if "order_id" in kwargs:
            data["client_order_id"] = kwargs["order_id"]
        order = await self._api_request("POST", "/api/v2/orders.json", data)
        return order

    async def cancel_order(self, order_id: str):
        await self._api_request("POST", f"/api/v2/order/delete.json?id={order_id}")

    async def get_order(self, order_id: str) -> dict:
        order = await self._api_request("GET", f"/api/v2/order.json?id={order_id}")
        return order

    async def get_open_orders(self, trading_pair: Optional[str] = None) -> dict:
        orders = await self._api_request("GET", "/api/v2/orders.json")
        if trading_pair is not None:
            orders = [order for order in orders if order["market"] == trading_pair]
        return orders

    async def get_closed_orders(self, trading_pair: Optional[str] = None, start_time: Optional[float] = None,
        ...
        if limit is not None:
            params["limit"] = limit
        trades = await self._api_request("GET", "/api/v2/trades/my.json", params=params)
        if trading_pair is not None:
            trades = [trade for trade in trades if trade["market"] == trading_pair]
        return trades
    ...
    async def _api_request(self, method: str, endpoint: str, params: Optional[dict] = None,
        ...
        url = f"https://api.{Constants.EXCHANGE_NAME}.com{endpoint}"
        auth_headers = self._peatio_auth.generate_headers(method, endpoint, params or {}, headers or {}, data or {})
        headers.update(auth_headers)

        while True:
            try:
                async with self.http_client.request(method, url, headers=headers, params=params, json=data) as response:
                    if response.status == 429:
                        ...
                        raise IOError(f"{response.status} {response.reason} - {err['error']}")
                    else:
                        return await response.json()

            except Exception as e:
                # logging.getLogger().error(f"Error making {method} request to {url}. Error: {str(e)}")
                await asyncio.sleep(5)
...
#     # def c_start(self, Clock clock, double timestamp):
#     #     self._tx_tracker.c_start(clock, timestamp)
#     #     ExchangeBase.c_start(self, clock, timestamp)

#     # def c_tick(self, double timestamp):
#     #     def:
...
#     #                       object amount,
#     #                       object price):
#     #     # Fee info from https://www.changeme.io/api/v2/peatio/public/trading_fees
...
# #         account_balances = await self._api_request("GET", path_url=Constants.GET_BALANCES)
...
#     #     current_tick = self._current_timestamp / self.UPDATE_ORDERS_INTERVAL
...
#                 raise
#             except Exception as e:
#                 self.logger().warning(f"Unexpected error while polling {Constants.EXCHANGE_NAME} updates. {e}",
#                                       exc_info=True)
#                 await asyncio.sleep(5.0)
...
#                 raise
#             except Exception as e:
#                 self.logger().warning(f"Unexpected error while fetching {Constants.EXCHANGE_NAME} trading rule updates. {e}",
#                                       exc_info=True)
#                 await asyncio.sleep(0.5)

#     # def OrderBook c_get_order_book(self, str trading_pair):
#     #     def:
...
#     # def c_start_tracking_order(self,
#     #                             str order_id,
...
#     # def c_stop_tracking_order(self, str order_id):
#     #     if order_id in self._in_flight_orders:
#     #         del self._in_flight_orders[order_id]

#     # def c_did_timeout_tx(self, str tracking_id):
#     #     self.c_trigger_event(self.MARKET_TRANSACTION_FAILURE_EVENT_TAG,
#     #                          MarketTransactionFailureEvent(self._current_timestamp, tracking_id))

#     # def object c_get_order_price_quantum(self, str trading_pair, object price):
#     #     def:
#     #         TradingRule trading_rule = self._trading_rules[trading_pair]
#     #     return Decimal(trading_rule.min_price_increment)

#     # def object c_get_order_size_quantum(self, str trading_pair, object order_size):
#     #     def:
#     #         TradingRule trading_rule = self._trading_rules[trading_pair]
#     #     return Decimal(trading_rule.min_base_amount_increment)

#     # def object c_quantize_order_amount(self, str trading_pair, object amount, object price=0.0):
#     #     def:
#     #         TradingRule trading_rule = self._trading_rules[trading_pair]
...
#         self.logger().info(f"Requesting {Constants.EXCHANGE_NAME} order placement for {order_id} at {self._current_timestamp}")
...
#     #     if decimal_amount < trading_rule.min_order_size:
#     #         raise ValueError(f"Sell order amount {decimal_amount} is lower than the {Constants.EXCHANGE_NAME} minimum order size "
#     #                          f"{trading_rule.min_order_size}")
...
# #             self.logger().error(
# #                 f"Failed to cancel {Constants.EXCHANGE_NAME} order {order_id}: {str(err)}.",
# #                 exc_info=True
# #             )
# #         return None

#     # def c_cancel(self, str trading_pair, str order_id):
...
#                 raise PeatioAPIException(f"Error fetching response from {http_method}-{url}. HTTP Status Code {response.status}: "
