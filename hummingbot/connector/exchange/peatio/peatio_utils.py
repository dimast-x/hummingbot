...
class PeatioConfigMap(BaseConnectorConfigMap):
    connector: str = Field(default="peatio", const=True, client_data=None)
    peatio_api_key: SecretStr = Field(
        default=...,
        client_data=ClientFieldData(
            prompt=lambda cm: "Enter your Peatio API key",
            is_secure=True,
            is_connect_key=True,
            prompt_on_new=True,
        ),
    )
    peatio_api_secret: SecretStr = Field(
        default=...,
        client_data=ClientFieldData(
            prompt=lambda cm: "Enter your Peatio API secret",
            is_secure=True,
            is_connect_key=True,
            prompt_on_new=True,
        ),
...
KEYS = PeatioConfigMap.construct()
...
#TRADING_PAIR_SPLITTER = re.compile("^\({Constants.TRADING_PAIR_SPLITTER}\)$")
TRADING_PAIR_SPLITTER = re.compile(r"^(\w+)(BTC|btc|ETH|eth|AUD|aud|PAX|pax)$")
